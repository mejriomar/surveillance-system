...
#define PASS_EEPROM_ADDR 500
int angle = 20;
int angle_up = 140;
// Fonction pour écrire une chaîne dans l'EEPROM
void writeStringToEEPROM(int addrOffset, const String &str) {
    byte len = str.length();
    EEPROM.write(addrOffset, len); // Stocke la longueur d'abord
    for (int i = 0; i < len; i++) {
      EEPROM.write(addrOffset + 1 + i, str[i]);
    }
    EEPROM.commit(); // Sauvegarde dans la mémoire flash
  }

    // Page HTML avec CSS et dropdown
...
<head>
  ...
  <title>Config Wi-Fi ESP32</title>
  <style>
    body {
...
String html_servo = R"rawliteral(
  <!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>ESP32-CAM Contrôle</title>
  <style>
    body {
      margin: 0;
      padding: 0;
...
<body>
  ...
  <div id="videoContainer">
    <img id="videoFeed" src="" alt="Flux vidéo ESP32-CAM" />
  </div>
  ...
  <div class="reset-button">
    <button id="reset_p" onclick="sendCommand('reset_p'">Reset</button>
  </div>

  <script>
    // Détection automatique de l’IP et affectation du flux vidéo
    const ip = window.location.hostname;
    document.getElementById("videoFeed").src = `http://${ip}:81/stream`;

    // Envoie la commande servo
    function sendCommand(dir) {
      fetch(`/servo?dir=${dir}`)
        .catch(() => alert("Commande échouée"));
    }
  </script>
...
String html_acc = R"rawliteral(
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>ESP32-CAM Home</title>
  <style>
    body {
      font-family: Arial, sans-serif;
    ...
    .button {
      ...
      border: none;
      border-radius: 8px;
      background-color:rgb(15, 168, 179);
      color: white;
    ...
    .button:hover {
      background-color: rgb(11, 218, 233);
    }
  </style>
</head>
<body>
  <h1>Gold-Tech-CAM Home</h1>
  <a class="button" href="/conf">Configuration</a>
  <a class="button" href="/wifi">Wi-Fi</a>
  <a class="button" href="/servo_ctr">Servo Control</a>
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    if (!strcmp(variable, "framesize")) {
        ...
        led_duty = val;
        Serial.println(String(led_duty));
        if (isStreaming)
            enable_led(true);
    }
#endif
...
static esp_err_t wifi_handler(httpd_req_t *req) {
  String html_page = html;  // Remplacer par ta page HTML réelle
  //html_page.replace("{options}", options); // options doit être une variable globale ou accessible ici

  httpd_resp_set_type(req, "text/html");
  httpd_resp_send(req, html_page.c_str(), html_page.length());
  return ESP_OK;
}

static esp_err_t reset_handler(httpd_req_t *req) {
  String html_page = "<h1>Bey!</h1>";  // Remplacer par ta page HTML réelle
  //html_page.replace("{options}", options); // options doit être une variable globale ou accessible ici
  httpd_resp_set_type(req, "text/html");
  httpd_resp_send(req, html_page.c_str(), html_page.length());
  ESP.restart();
...
static esp_err_t servo_ctr_handler(httpd_req_t *req) {
  String html_page = html_servo;  // Remplacer par ta page HTML réelle
  //html_page.replace("{options}", options); // options doit être une variable globale ou accessible ici
...
static esp_err_t acc_handler(httpd_req_t *req) {
  String html_page = html_acc;  // Remplacer par ta page HTML réelle
  //html_page.replace("{options}", options); // options doit être une variable globale ou accessible ici
...
esp_err_t servo_handler(httpd_req_t *req) {
  ...
  if (httpd_query_key_value(buf, "dir", dir, sizeof(dir)) == ESP_OK) {
    Serial.printf("Commande servo revue : %s\n", dir);

    // Exemple : contrôle des deux servos selon la direction
    if (strcmp(dir, "up") == 0) {
       angle_up = angle_up + 20;
       if(angle_up >180)
       {
        angle_up = 180;
       }
       moveServo(angle_up,2);
      // servo1.write(angle haut);
      ...
      // servo1.write(angle bas);
       ...
       moveServo(angle,1);
    } else if (strcmp(dir, "led_on") == 0) {
      ledcWrite(LED_LEDC_CHANNEL, 200);
      Serial.println("led on");

    } else if (strcmp(dir, "led_off") == 0) {
      Serial.println("led off");
      ledcWrite(LED_LEDC_CHANNEL, 0);

    }else if (strcmp(dir, "reset_p") == 0) {
        moveServo(140,2);
        moveServo(20,1);
    }
    Serial.printf("angle_up = %d\n",angle_up);
...
// Handler pour recevoir les paramètres du formulaire HTML
esp_err_t save_handler(httpd_req_t *req) {
  ...
  if (buf_len > 1) {
    ...
    if (httpd_req_get_url_query_str(req, query, buf_len) == ESP_OK) {
      ...
      Serial.printf("SSID reçu: %s\n", ssid);
      Serial.printf("Mot de passe reçu: %s\n", pass);
      writeStringToEEPROM(SSID_EEPROM_ADDR, ssid);
      writeStringToEEPROM(PASS_EEPROM_ADDR, pass);

    }
    free(query);
  }

  const char* resp_str = "<h3>Wi-Fi recu. Redemarrage...</h3>";
  httpd_resp_send(req, resp_str, HTTPD_RESP_USE_STRLEN);

  delay(2000);
  ESP.restart();  // Redémarrage après réception
...
void startCameraServer()
{
    ...
    httpd_uri_t servo_ctr_uri = {
        .uri = "/servo_ctr",
        .method = HTTP_GET,
        .handler = servo_ctr_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &servo_ctr_uri);
        httpd_register_uri_handler(camera_httpd, &acc_uri);
        httpd_register_uri_handler(camera_httpd, &servo_uri);
        httpd_register_uri_handler(camera_httpd, &ip_uri);
